...
%Si lamusicothérapie a un impact certain sur la façon d'écouter en
%entraînant sa modification, est-elle démontrable, \textsl{objectivable},simplement sous la forme 
%d'untest, comme saisieparl'\oe il neutrde l'objectif d'un appareilphotographique?
...
%Ecouter ou entendre : une différence
...
\end{quote}
Selon Auriol % \autocite[18] {auriol:cle}
 et Tomatis, l'écoute est un `` éveil auditif''  défini avec au
minimum trois
fréquences simultanées dans le sens esthétique. % \autocite%[52]
%{tomatis_oreille_1987}. 
...
%Dans un milieu sonore important,% bruyant, comme un café, lorsque nous lisons attentivement, 
%%%%noufaisons abstraction
%des bruits environnants; en soi, nous les entendons parfaitement mais nous n'y%prêtons pas attention.
...
%par sa capacité d'écoute affinée à l'extrême 
%W. Fasser est devenu, en affinant sa capacité d'écoute à l'extrême, un explorateur des sons de 
%l'univers,  
%un 
 %\enquote{cueilleur de sons} pour entrer encore mieux en résonance avec ses patients. 
%La sensibilisation aux perceptions sensorielles 
%Nous rejoignons ainsi  le concept de 'communication' abordé 
%plus haut.
%\textbf{Ecoute objective ou subjective?}
\\
...
en revue les recherches qui existent  au sujet de la relation émotion, musique et cerveau. %approfondir 
%ce qu'il en est des recherches 
 

%De ce rôle du cerveau éminemment complexe, nous avons mentionné dans  et de son pouvoir de 
%sélectivité
%Le cerveau joue  donc un rôle Voyons brièvement ce qu'il en est des incessantes recherches 
%scientifiques actuelles. que ce soit  dans la façon de 
%sélectionner les sons mais aussi dans son rapport avec la musique. Voyons brièvement ce qu'il en est 
%des recherches scientifiques actuelles.
\subsection{ L'écoute dans le rapport musique-cerveau}
...
récentes.
% ainsi que Canbeyli \autocite{Canbeyli}.
% Sackheim & Epstein et al.
%\footnote{Yovell
...
\subsection{Rapport entre audition et émission vocale}
...
Ainsi, le concept de `résonance' s'applique non seulement aux instruments mais aussi au corps humain. 
...
%ce lien puissant et indissociable, est attestée par t
\\
...
recherches. 
%Toutes ces recherches nous permettent d'aller dans
%le sens de notre étude tout en appuyant
%la fonction thérapeutique de la musique.
%Ainsi ce  concept extrêmement important de résonance se voit non seulement utilisé avec les 
%techniques 
%vocales en musicothérapie mais aussi  recherché  dans le domaine des techniques de la 
%musicothérapie 
%que constaté dans celui de la recherche. 

%Ainsi ce  concept extrêmement important de résonance se voit non seulement appliqué non seulement 
%dans le domaine 
%des techniques en musicothérapie mais se trouve aussi constaté comme élément central dans les 
%domaines 
...
%\paragraph{Définition et différence entre test d'écoute et audiogramme: } 
...
En ce qui concerne les tests d'écoute en musicothérapie, nous constatons que les psychanalystes, les 
...
explorent l'alliage du son
  dans le domaine de la psychologie.%et inversément, les psychanalystes, les psychiatres, les 
  %psychologuess'intéressent à intégrer le son dans leur travail.
...
L'ouvrage \enquote {\textit{Le bilan musical et la personnalité}}\autocite{verdeau-pailles:bilan}
                     est un outil qui permet, en partant des \enquote {liens personnels avec le sonore}, d'obtenir 
...
\autocite[39]{vrait_musicotherapie_2018}.% Cf. fig. \ref{morceauxDeMusique}.
...
%\footnote{D. W. Winnicott: ``Jeu et% réalité''}
 %et Watzlawick \autocite{Watzlawick}.%   Celui-ci se déroule soit avec l'audition d'\oe uvres
         ...
         % une écoute musicale (partie réceptive) et une production musicale
          %(partie active)%        \subsection{Benenzon Rolando Omar}
	%\textbf{ Psychiatre et psychanalyste,
  %  musicien et musicothérapeute}, né en 1939 à Buenos Aires.
	  %\label{benenzon}
%\footnote{ Watzlawick Paul, 1921-2007  théoricien dans la théorie de la communication et le 
%constructivisme radical, membre fondateur de l'École de Palo Alto, psychologue, psychothérapeute, 
%psychanalyste jungien et sociologue}
...
%\textbf{Nevjinsky}\autocite{nevjinsky:adolescence}, \textbf{ Auriol}\autocite{auriol:cle},
        %\textbf{Bonhomme}\autocite{bonhomme-key}
                  % et \textbf{Tomatis}\autocite{tomatis_oreille_1987} ont également, de leur côté, développé leurs propres
...
%Mozart, Rachmaninov ou Vivaldi, mais il ne s'agit pas d'un test à proprement parler.%\subsection{ 
%%Nevjinsky Fern}
...
L'introduction des morceaux de musique en association libre (avec
 ...
 souvenirs. %Malheureusement et malgré nos recherches, il nous manque des détails, mais nous savons 
 %qu'il était convaincu qu'un test ne pouvait se limiter à des sons purs.
...
% comme on le verra plus loin (Cf. Ch. 3. 3.),
...
%pouvant contribuer à un diagnostic également intéressant.
 %\subsection{ Bonhomme Jacques }
  ...
  %émotionnels.
 % Il nous livre ses 11 clés phonatoires pour refaire circuler dans %\textit{``La voix est la
...
\enquote {Les avancées de la physiologie cochléaire et de la neuro-psychologie associées aux données 
...
caractéristique de la transformation de l'écoute. %mettant en valeur : l'écoute n'est pas statique, elle se 
%transforme, 
% et s'ouvre de plus en plus sur d'autres sur un très large horizon.
%Auriol a conçu de nombreux tests d'écoute, --- non illustrés ici --- à partir de celui de 
%Tomatis,  
...
% test 
%qui %diffère du test d'audition (physiologique). La musicothérapie porte actuellement son attention 
%%moins 
%sur %les effets généraux de la musique  mais beaucoup plus directement sur le patient et sa 
%%souffrance.
%Nous retiendrons le caractère tout de même un peu sélectif avant même le début 
%d'une 
%musicothérapie du test de Verdeau-Paillès, malgré tous les avantages 
%évoqués,  nécessitant d'autre part la présence de plusieurs spécialistes pour sa lecture.  
%, il en existe probablement d'autres  en Angleterre, en Allemagne ou dans d'autres pays. 
Il nous importait ainsi de rechercher dans le domaine de la musicothérapie le concept de "test" dont 
nous 
avons dressé une  liste non exhaustive, % des tests retenue dans le domaine de la musicothérapie
 afin de mieux 
...
une meilleure
captation des sons. % \autocite{tomatisassociation.org}.
