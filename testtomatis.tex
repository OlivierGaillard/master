% !TEX root = ./master.tex
\subsection{Technique de passation du test Tomatis\textsuperscript \textregistered }
...
%niveau de l'os mastoïde transmettant à son tour à  la voie nerveuseauditive.
%Parmi les quelques éléments différentiels
%apparaissant par la suite dans les observations cliniques, 
\\
...
et osseuse, permettant de relever les paramètres d'harmonie ou
 ...
 %	On parlera	d'équilibre ou de déséquilibre, d'harmonie ou de disharmonie.	
 	 \\
 	En résumé: 
        \begin{enumerate}
...
de couleurs différentes, permettant de faire une comparaison avec la courbe
 ...
 % (Fig. 3.7),dite de Caruso.(Cf.Caruso, Ch. 3. 1.)
...
%courbe dénommée idéale (Cf.Caruso, Ch. 3. 1.)
...
%\begin{itemize}
%\item La conduction aérienne : traduit la vie sociale, la manière de communiqueret de s'extérioriser.
...
%de l\textquoteright auto-écoute, de l\textquoteright auto-contrôle,de l'écoute intérieure.
%\end{itemize}
...
%Situées en CO, ce sont des tensions internes non exprimées : attitudecalme mais très tendue 
%intérieurement.
%Situées en CA, ce sont des tensions réelles et exprimées au quotidien: soit somatisées, verbalisées ou 
%manifestées sur le planaffectif.
...
de chaque diagramme. Issus des recherches cliniques de Tomatis, %dans le domaine empirique,
...
% Les fréquences se répartissent desgraves aux aigües:
\begin{itemize}
...
%Pour décrire plus concrètement le paragraphe précédent, nous avons dressé un portrait type et général 
%en ressortissant les liens et correspondances possibles entre les qualités d'écoute des patients, leur 
%expressionmusico-phy\-sico-psy\-cho\-lo\-gi\-que et leur trouble de l'humeur négative. 
%Lors de ce travail, nous avons ressorti les aspects et  les techniques utilisées, ressorti  les liens et 
%correspondances possibles avec les qualités d'écoute des patients et leur humeur négative.  %entre la 
%%voix,  %--d'ailleurs utilisée dès les premières conversations et 
%échanges--,
...
physico-psychologique de ce type de population souffrant des troubles de l'humeur, 
  %et plus particulièrement sur celle de 
%la zone consacrée
...
fréquemment relevé chez ces patients. %semble avoir tout son sens dans ce contexte. 



%\paragraph{Descriptif des zones d'interprétation}
...
corps se redresse.
Le mouvement détend et la suggestion d'image amuse. A la délicieuse odeur d'un gâteau au 
...
la moyenne oreille droite/gauche. %Afin de rentrer plus dans les détails, l'oreille droite sera d'abord 
%examinée. 
Ces 
...
%Données amnamestiques: âge, profession, famille etc., raison d'hospitalisation\textbf{ Test d'écoute 
%avant les séances de musicothérapie:}
Anamnèse: le patient, %née en 1981,
...
\subsection{Déroulement général de deux séances individuelles de musicothérapie} 
...
patients (conversation et réactions);  %test effectué deux fois, 
%en début et 
%fin de thérapie
...
%, effectué deux fois, en début et fin de séjour en clinique.
...
%cette clinique, ainsi chaque patient a un suivi individuel.  % de 
%patients de la 
...
%- 
%thérapie.
%Les outils pour récolter les données sont constitués par le test d'écoute Tomatis, le questionnaire 
%WHOQOL sur la qualité de vie et par les observations des séances en musicothérapie.%et les 
%%techniques  
%musicothérapeutiques observées sur l'écoute.
%????---- Le test d'écoute est  un outil d'observation comparatif  de la capacité d'écoute en début et en 
%fin de thérapie, nous donnant également des repères dans le suivi et l'aboutissement du 
%processusmusicothérapeutique.
% \enquote {avec un souci de rigueur dans une 
%	perspective pragmatique et claire du point de vue clinique et institutionnel}\autocite[p. 
%36]{vrait_musicotherapie_2018}. 
%????-----Il a lieu 1 fois en début et 1 fois en fin de séjour clinique; il s'agit de la reconnaissance par le 
%patient  de sons spécifiques (de 8000 à 250 Hz) émis à faible volume avec augmentation progressive. 
...
% (Cf. Ch.3, 3.2.2 Technique de 
%passation du test d'écoute). 
% \\
%----????De même avec le questionnaire WHOQOL, rempli par le patient en début et fin de séjour, qui 
%permet de comparer le ressenti subjectif de la qualité de vie.
%\\
...
En plus de l'administration et l'intendance, les 33
 ...
 musicothérapie.
 %avec deux personnes, musicothérapeute  à 90\%  à la clinique et  l'auteure de ce travail avec un 
 %remplacement fixe et régulier à 10 \%.
 ...
 patients, explique le projet de l'étude. % sur la capacité d'écoute, ainsi que leur possible 
 %transformation,avec ou sans musicothérapie.
 ...
 La population étudiée a été constituée par un nombre de patients groupé pour une musicothérapie 
 ...
 %en deux groupes dont le choix des thérapi était dicté  constitués selon une technique de choix des 
% th\footnote{On m'a prié de ne pas donner davantage de précisions}  % selon les directives 
 %inhérentes à 
 %la clinique.des thérapies créatives%, 
 %%selon les critères intrinsèques à  la clinique..
  Le critère de sélection était l'obtention
 d'un nombre suffisant de 
 patients en musicothérapie.
 %La musicothérapie en groupe n'existant pas dans la clinique, chaque patient aura  un suivi individuel, à 
 %la fréquence d'une fois par  semaine.
  ...
  % d'une 
 % comparaison nécessaire à cette étude imposant la réalisation des tests et 
  %questionnaires à l'entrée et à la sortie de clinique.
 %\textbf{ Pathologie des groupes}:
 % dont l'un est témoin. En ce qui concerne la 
%  \\ Nous avons conscience d'avoir mélangé des symptomatologies qui toutefois 
%paraissentsous-tendues parcertains mécanismessimilaires dont lenoyau commun est unedifficulté de	
%régulation desémotions,s'exprimant  par une humeur négative dont le burn-out, les dépendances, la 
%dépression.
...
% se trouvent traitées ensemble. \\
 ...
 %La musicothérapie dans cette clinique consiste en un suivi individuel,
  % Le  nombre de participants au groupe de musicothérapie (GM)  a été  constitué  et planifié en début   
  %de séjour 
   %grâce à l'aide de Regula Lehman.
   %, au début de leur 
 % séjour.  Il se passe  sous  
 ...
 % être divulgués ici.
  ...
  %Les participants du groupe de contrôle (GC) sans musicothérapie  ont été sélectionnés au début de 
  %leur séjour selon leur souhait de participation au test de capacité d'écoute.
...
%  il en a été de même lors des tests de capacité d'écoute.
 ...
 %, de Hegi.
 % à savoir la possibilité de procéder à celle-ci, la \textbf{ comparaison pré/ post -- thérapie} avec tests 
 %et  
%questionnaires. Toutes ces contraintes ont  eu un impact non négligeable sur le nombre de patients à 
%étudier. 
% Ainsi nous avons fait, en plus des tests présentés, 18  tests  
 %inutilisables car sans comparaison possible avec ceux qui auraient dû être faits en fin de séjour ni en 
 %lien  avec le WHOQOL.
...
\begin{figure}
	\centering
	\includegraphics[width=0.7\linewidth]{images/testecoute.jpg}
...
\\
% \begin{itemize}		
%	\item la comparaison des graphiques des différentes courbes: allure générale, symétrie, 
...
%Nous avons simplifié les résultats
forme 
...
%	représentée sous forme de la  courbe aérienne
%	\item   les \textbf{seuils} --moyenne
%	représentée sous forme de la courbe osseuse
%\end{itemize}
\subsection {Considérations générales des données, observation et interprétation: }
